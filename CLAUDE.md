...
`content/traditions/` and `content/verses.json` are generated too.
...
- `2f1669a87b75d52c2e8e87c9d5f864f57a6b2da9483e8f42ff390aaebe8b8797` as written by
  `Path.write_text` on Windows (CRLF)
- `018a5dbc3bf4d02dc5a71b512f857942ee14e5e617afa96e2ecb4175f1bf2ec8` LF-normalised (what a
...
- **`content/verses.json`** — generated by `py engine/render.py` (`write_verses_json`),
  **never hand-edited**. `documentation/verses.md`'s text as
  `{ translation: "NET", verses: { ref: text } }`, references with no text omitted. It
  exists because `web/` had no client-reachable copy of `verses.md` at all; `/learn`'s
  verse popups fetch it through `web/corpus.js`'s `loadVerses()`.
