...
<style>
  ...
  * { box-sizing: border-box; }
  body { margin:0; background:var(--bg); color:var(--ink);
         font: var(--fs-1)/var(--lh-body) var(--serif); text-rendering:optimizeLegibility; }
  header { position:sticky; top:0; z-index:10; background:var(--bg);
           border-bottom:1px solid var(--line); padding:var(--s4) var(--s5) var(--s3); }
  .kicker { margin:0 0 var(--s1); font:600 var(--fs-000)/var(--lh-ui) var(--sans); letter-spacing:.16em;
  ...
  h1 { margin:0; font-family:var(--serif); font-size:var(--fs-3); letter-spacing:-.012em;
    font-weight:600; }
  ...
  .titlerow { display:flex; align-items:center; gap:var(--s3); flex-wrap:nowrap; margin:0 0 var(--s1); }
  .titlefilters { display:flex; align-items:center; gap:var(--s2); margin-left:auto; min-width:0; }
  .sub { color:var(--muted); font:var(--fs-00)/var(--lh-ui) var(--sans); margin-bottom:var(--s3); max-width:var(--measure); }
  .bar { display:flex; gap:var(--s4); align-items:center; flex-wrap:wrap; font-family:var(--sans); }
  /* .views/.seg's gap:2px and padding:3px are a segmented-control inset, not
     a spacing step — frozen. */
  .views { display:flex; gap:2px; background:var(--chip); padding:3px; border-radius:var(--r2); }
  .views button { border:0; background:transparent; color:var(--muted); cursor:pointer;
    padding:var(--s1) var(--s3); border-radius:var(--r2); font:600 var(--fs-00)/var(--lh-tight) var(--sans); letter-spacing:.01em; }
  .views button[aria-pressed="true"] { background:var(--panel); color:var(--ink);
    box-shadow:var(--shadow); }
  .seg { display:flex; gap:2px; background:var(--chip); padding:3px; border-radius:var(--r2); }
  .seg button { border:0; background:transparent; color:var(--muted); cursor:pointer;
    padding:var(--s1) var(--s3); border-radius:var(--r2); font:600 var(--fs-00)/var(--lh-tight) var(--sans); }
  ...
  input[type=search] { border:1px solid var(--field-line); background:var(--panel); color:var(--ink);
    padding:var(--s2) var(--s3); border-radius:var(--r2); font:var(--fs-00)/var(--lh-tight) var(--sans); min-width:190px; }
  input[type=search]::placeholder { color:var(--muted); }
  label.tog { font:var(--fs-00)/var(--lh-tight) var(--sans); color:var(--muted); display:flex; gap:var(--s1);
    align-items:center; cursor:pointer; }
  label.tog input { accent-color:var(--accent); }
  .btnrow { display:flex; gap:var(--s2); }
  .btnrow button { font:600 var(--fs-000)/var(--lh-tight) var(--sans); border:1px solid var(--field-line);
    background:var(--panel); color:var(--muted); padding:var(--s1) var(--s3); border-radius:var(--r2); cursor:pointer; }
  ...
  summary:focus-visible, [tabindex]:focus-visible {
    outline:2px solid var(--muted); outline-offset:2px; border-radius:var(--r1); }
  main { padding:var(--s4) var(--s5) var(--s8); max-width:1080px; margin-inline:auto; }
  main.wide { max-width:none; padding:var(--s4) var(--s4) var(--s4); }
  .group { margin-bottom:var(--s5); }
  .group > h2 { font:600 var(--fs-000)/var(--lh-ui) var(--sans);
    color:var(--muted); margin:0 0 var(--s3); cursor:pointer; user-select:none;
    display:flex; align-items:center; gap:var(--s2); flex-wrap:wrap;
    padding-bottom:var(--s2); border-bottom:1px solid var(--line); }
  .group > h2:hover { color:var(--ink); }
  .group > h2 .chev { display:inline-block; transition:transform .15s ease; font-size:var(--fs-000);
    color:var(--muted); }
  ...
  .chip { font:600 var(--fs-000)/var(--lh-ui) var(--sans); color:var(--muted);
    ...
    padding:3.5px 6.5px; border-radius:var(--r1);
    background:var(--chip); }
  .chip.tier { color:#fff; }
  ...
  /* 58ch is the prose cap from phase 3 §3. min() keeps it from overflowing a
     ...
     box. */
  dd { margin:0; font:var(--fs-0)/var(--lh-body) var(--serif); max-width:min(var(--measure), 100%); }
  dd.todo { color:var(--muted); font-style:italic; }
  dd.refs, dd.rel { max-width:none; display:flex; flex-wrap:wrap; gap:var(--s2); align-items:center; }
  .refchip { font:500 var(--fs-000)/var(--lh-ui) var(--mono); letter-spacing:.01em;
    color:var(--muted); background:transparent; border:1px solid var(--field-line);
    /* padding is an optical value on a 1px-bordered micro-pill, not a
       spacing step — frozen. */
    padding:2.5px 7px; border-radius:var(--r1); white-space:nowrap;
  ...
  .versepop { position:fixed; z-index:100; max-width:min(46ch, calc(100vw - 28px));
    ...
    border-radius:0 var(--r3) var(--r3) 0; box-shadow:var(--shadow);
    padding:var(--s3) var(--s4) var(--s3); }
  .versepop-head { font:600 var(--fs-000)/var(--lh-ui) var(--sans);
    color:var(--muted); margin-bottom:var(--s2); padding-bottom:var(--s2); border-bottom:1px solid var(--line); }
  .versepop-body { font:var(--fs-0)/var(--lh-body) var(--serif); color:var(--ink); }
  .versepop-empty { font:var(--fs-00)/var(--lh-ui) var(--sans); font-style:italic; color:var(--muted); }
  .versepop-attr { margin-top:var(--s2); font:var(--fs-000)/var(--lh-ui) var(--sans); color:var(--muted); }

  .pagefoot { max-width:1080px; margin:0 auto; padding:var(--s4) var(--s5) var(--s7);
    font:var(--fs-000)/var(--lh-ui) var(--sans); color:var(--muted); }
  .pagefoot a { color:inherit; }
  dd.rel a { font:var(--fs-000)/var(--lh-ui) var(--sans); color:var(--muted); text-decoration:none;
    border:1px solid var(--field-line); padding:var(--s1) var(--s2); border-radius:var(--r2);
    cursor:pointer; }
  ...
  .legend { display:flex; gap:var(--s3); flex-wrap:wrap; margin-top:var(--s3);
    font:var(--fs-000)/var(--lh-tight) var(--sans); color:var(--muted); }
  ...
  /* Tier colour is never the sole channel (phase 3 §3.2, failure B). Each
     ...
     The 3px radius is a measure on a 10px swatch, not a spacing step, and
     stays numeric. */
  ...
  .mmeta {
    display:flex; gap:var(--s1); flex-wrap:wrap; margin-top:var(--s2);
  ...
  .mapcontrols button { font:600 var(--fs-000)/var(--lh-tight) var(--sans); border:1px solid var(--field-line);
    background:var(--panel); color:var(--muted); padding:var(--s1) var(--s2); border-radius:var(--r2); cursor:pointer;
    box-shadow:var(--shadow); }
  ...
  /* ---------- phone layout ---------- */
  @media (max-width:640px) {
    header { padding:var(--s3) var(--s4) var(--s2); }
    .kicker { display:none; }
    h1 { font-size:var(--fs-2); }
    .sub { display:none; }
    .legend { display:none; }
    .titlerow { gap:var(--s2); }
    .bar { gap:var(--s2); row-gap:var(--s2); }
    .viewrow { gap:var(--s2); width:100%; }
    .views { overflow-x:auto; -webkit-overflow-scrolling:touch; flex:0 1 auto; scrollbar-width:none; mask-image: linear-gradient(90deg, #000 calc(100% - 24px), transparent); }
    ...
    /* Map-first (phase 1.5): one header row -- the view switcher and the
       Filters toggle. The h1 is visually hidden (the tab title, and /view's own
       chrome, carry it); search lives inside the Filters disclosure, and the
       toggle carries a dot while a search is live so a hidden filter is never
       a silent one. display:contents lets the existing markup take part in one
       grid without moving a node. */
    header { display:grid; grid-template-columns:minmax(0, 1fr) auto; align-items:center;
      gap:var(--s2); padding:var(--s2) var(--s3); }
    .titlerow, .titlefilters, .bar { display:contents; }
    h1 { position:absolute; width:1px; height:1px; overflow:hidden;
      clip-path:inset(50%); white-space:nowrap; }
    .viewrow { grid-column:1; grid-row:1; }
    .filtersToggle { grid-column:2; grid-row:1; }
    input[type=search]#q { grid-column:1 / -1; display:none; width:100%; box-sizing:border-box; }
    header:has(.secondary.open) input[type=search]#q { display:block; }
    header:has(#q:not(:placeholder-shown)) .filtersToggle::after { content:"\\2022"; color:var(--ink); }
    .secondary { grid-column:1 / -1; }
    #mapwrap { border:0; border-radius:0; }
  }

  @media (max-width:560px) {
    dl { grid-template-columns:1fr; gap:var(--s1) 0; }
    dt { padding-top:var(--s2); }
...
function gotoNode(slug) {
  ...
  requestAnimationFrame(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (el) {
        // F2: the behavior option overrides the computed scroll-behavior, so
        // the scroll-behavior: auto !important reduced-motion guard elsewhere
        // in this file cannot reach it -- this needs its own guard.
...
// The header's height changes without a window resize -- below 640px the
// Filters disclosure (which holds the search field) opens and closes inside it.
// Without this the canvas kept the height it was given while the disclosure
// was open, and every phone search left the map a third shorter.
if (window.ResizeObserver) new ResizeObserver(sizeMap).observe(document.querySelector('header'));
