# debug-archive.md — the full bug write-ups (Project 12)

**This file is not read by default.** `debug.md` carries the lessons; this carries the
narratives behind them. Come here when a rule in `debug.md` is not enough and you need
to know exactly what happened, what was tried, and why the fix is shaped the way it is.

Entries run A-AK in the order they were found, phase 1 (2026-08-22) to the ponytail
audit (2026-09-05). Every one is FIXED unless its heading says otherwise. Several
describe files that no longer exist (`web/index.html`, `web/first-run.js`, `/app`) —
kept because the *seam* each one describes is still reachable in the files that
replaced them.
